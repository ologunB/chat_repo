...
class AppProvisioning {
  ...
  /// URL used for getting token request
  String _realTokenRequestAuthUrl(String name) =>
      'https://rest.ably.io/keys/$name/requestToken';
  ...
  Future<Map<String, dynamic>> getToken(List<String> keys) async {
    String username = keys.first;
    String password = keys.last;
    String basicAuth =
        'Basic ${base64.encode(utf8.encode('$username:$password'))}';
    final response = await _httpRetryClient.post(
      Uri.parse(_realTokenRequestAuthUrl(username)),
      headers: {'authorization': basicAuth},
      body: {
        "keyName": username,
        'clientId': AppCache.getUser(),
